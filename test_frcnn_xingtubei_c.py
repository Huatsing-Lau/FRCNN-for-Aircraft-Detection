...
import argparse
import os
import keras_frcnn.resnet_SA_CNN as nn
...
def predict_single_image(img_path, model_rpn, model_classifier_only, cfg, class_mapping):
    ...
    result_path = './results_images/{}.png'.format(os.path.basename(img_path[:-4])) #.split('.')[0]
...
def predict(args_):
    ...
    print('cfg.anchor_box_scales',cfg.anchor_box_scales)
    print('cfg.anchor_box_ratios',cfg.anchor_box_ratios)
    ...
    shared_layers,_,_ = nn.nn_base(img_input, trainable=True)
...
def parse_args():
    ...
    parser.add_argument('--path', '-p', default='../DOTA_devkit/xingtubeisplit/test/images_split', help='image path')#/000010.png
