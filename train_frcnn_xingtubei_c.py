...
这个函数是根据吴佳祥同学硕士毕业论文改进的faster-rcnn在星图杯数据集上的训练
"""
...
from keras_frcnn import losses_SA_CNN as losses_fn
import keras_frcnn.roi_helpers as roi_helpers
from keras.utils import generic_utils
import os
from keras_frcnn import resnet_SA_CNN as nn
...
def train_fasterrcnn():
    ...
    cfg = config.Config()
    cfg.balanced_classes = False
    cfg.use_horizontal_flips = True
    cfg.use_vertical_flips = True
    cfg.rot_90 = True
    cfg.num_rois = 100 #50# 对于星图杯的光学遥感飞机检测，应该改为50+
    cfg.anchor_box_scales = [16,32,64,128,256]#[41,70,120,20,90]
    cfg.anchor_box_ratios = [[1,0.7],[1,1.4],[1,1],[1.4,1],[0.7,1]]#[[1,1.4],[1,0.84],[1,1.17],[1,0.64],[1,1]]
    cfg.rpn_stride = 8
    cfg.im_size = 512
    cfg.num_epochs = 100
    ...
    shared_layer,shared_layers_stage3,shared_layers_stage4 = nn.nn_base(img_input, trainable=True)# shared_layers是frcnn网络底部那些共享的层,在这里是ResNet。由nn定义好

    # define the RPN, built on the base layers
    num_anchors = len(cfg.anchor_box_scales) * len(cfg.anchor_box_ratios)
    rpn_stage3 = nn.rpn(shared_layers_stage3, num_anchors)
    print(rpn_stage3[1].shape)
    rpn_stage4 = nn.rpn(shared_layers_stage4, num_anchors) # [x_class, x_regr, base_layers]
    ...
    classifier = nn.classifier(shared_layer, roi_input, cfg.num_rois, nb_classes=len(classes_count), trainable=True)

    model_rpn = Model(inputs=img_input, outputs=rpn[:2])#rpn网络由keras_frcnn/resnet定义。rpn[:2]的前两个元素分别表示rpn网络的分类输出和回归输出
    model_classifier = Model(inputs=[img_input, roi_input], outputs=classifier)#Keras的函数式模型为Model，即广义的拥有输入和输出的模型

    # this is a model that holds both the RPN and the classifier, used to load/save weights for the models
    model_all = Model(inputs=[img_input, roi_input], outputs=rpn[:2] + classifier)#rpn[:2]+classifier的含义是？？？？？？
    ...
    best_loss = 5.0 #2.1 #np.Inf # 训练以来最小的loss
